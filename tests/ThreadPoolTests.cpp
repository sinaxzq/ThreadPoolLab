...
void testShutdownCanBeCalledMultipleTimes()
{
    ...
    const bool accepted = pool.submit([](int workerId) { (void)workerId; });
...
void testSubmitFutureReturnsTaskResult()
{
    ...
    auto stringFuture = pool.submitFuture(
        [](int workerId) { return std::string{"worker "} + std::to_string(workerId); });
